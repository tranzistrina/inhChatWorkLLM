...
from workmode import ingest_uploads,source_context
...
def task(cid):
 ...
 prompt=[{'role':'system','content':'Execution stage. Execute ONLY the assigned task. You have source files and previous task results. If you need to create a text artifact, output it exactly as FILE:path on one line followed by its full content. Do not execute other tasks.'},{'role':'user','content':f'Original request:\n{r["request"]}\n\nAssigned task:\n{json.dumps(tasks[idx],ensure_ascii=False)}\n\nSource files:\n{source_context(src)}\n\nPrevious results:\n{previous}'}]
...
def final(cid):
 ...
 prompt=[{'role':'system','content':'Final synthesis stage. Prepare the final answer to the user from completed task results. Do not invent work. Mention generated files.'},{'role':'user','content':r['request']+'\n\nSOURCE FILES:\n'+source_context(src)+'\n\nCOMPLETED TASKS:\n'+work}]
