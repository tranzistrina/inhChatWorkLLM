...
def strict_system_prompt(allow_invention=False):
    return json.dumps({'rules':STRICT_RULES,'schema':{'title':'ОТЧЕТ','discipline':'string','lab_number':'string','topic':'string','student':'string','group':'string','city':'string','year':'string','sections':[{'heading':'string','paragraphs':['string'],'steps':['string'],'tables':[[['cell']]],'figures':[{'reference':'string','caption':'string','screenshot_description':'string'}],'answers':['string']}]},'constraints':['Return ONLY valid JSON.','Use only supplied sources and user conditions.','Never invent measurements or screenshots unless invention is explicitly enabled.','If invention is enabled, label every invented value as illustrative/example/assumption and never present it as observed.','For unavailable screenshots create a placeholder and caption plus a detailed screenshot_description.','Preserve source order and terminology.','INVENTION_ENABLED='+str(bool(allow_invention))]},ensure_ascii=False)